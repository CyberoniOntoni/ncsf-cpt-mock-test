...
export default async function PortalFindPage({
  searchParams,
}: {
  ...
  const { seeker } = await requireCompleteSeeker("/portal/find");
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-semibold">Find a trainer</h1>
        <p className="mt-1 text-sm text-zinc-400">{SITE_DISCLAIMERS.findIntro}</p>
      </div>
      <FindSearch
        gyms={gyms}
        brands={brands}
